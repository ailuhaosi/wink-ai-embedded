...
通道 3（模拟量）在 [`08-channel-routing.md §2.4`](../../design/04-wasm-simulation/02-mechanisms/08-channel-routing.md) 中长期处于 **Planned**：其架构铁律已写明——保留 DAL 的校准/滤波/阈值/错误码同源，旁路只替换"ADC 原始值来源"，插件注入"绑定到 ADC 通道的物理源"，禁止直接 `return temperature_c`。PinArbiter 也已具备模拟通道能力（`setAnalogDriver`/`readAnalog → [0,1]`，见 [`07-peripheral-registry.md §4.3`](../../design/04-wasm-simulation/02-mechanisms/07-peripheral-registry.md)），退化引擎已有 `adc_noise_v` setter 与 `wink_phys_rc_lowpass`/`wink_phys_warmup_check` 算法。
