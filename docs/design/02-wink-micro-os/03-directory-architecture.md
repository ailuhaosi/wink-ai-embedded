...
│   │   ├── pal_rmt_esp32.c            #   HAL RMT（超声波脉冲捕获）
...
| **外设/子模块拆分 TU**（domain 内按外设拆 file） | `pal_<domain>_<detail>_<plat>.c` | `pal_hal_gpio_esp32.c`、`pal_hal_i2c_esp32.c`、`pal_hal_pwm_esp32.c`、`pal_rmt_esp32.c` | — | （host 暂不拆，RMT/pulse_in 等都聚合在 pal_hal_host.c 内） |
