...
自动化文档契约与黑盒隔离校验脚本：
1. 校验 docs/design/ 下的规范文件，断言不得泄露外仓私有源码路径 (如 src/views/..., src/components/...)。
...
def check_essential_files():
    essential_files = [
        ...
        os.path.join(DESIGN_DIR, "00-quick-start", "01-5min-getting-started.md"),
        os.path.join(DESIGN_DIR, "README.md"),
    ]
    missing = []
    for path in essential_files:
        if not os.path.exists(path):
            missing.append(os.path.relpath(path, WORKSPACE_DIR))
    return missing
