...
4. **Host <-> Wasm consistency (section 9.8):** same unbound call must fail observably on both targets.
...
- Use **claim return value** (`BUSY` = foreign owner) to check conflicts.
- **Prerequisite:** enable real `pal_resource_*` on wasm.
