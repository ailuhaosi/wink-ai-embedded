...
| 模块 | 范围 |
|---|---|
| 目标板 | ESP32 DevKit V1 / STM32F4 / Host Wasm |
| 外设 | LED、Button、RC/Industrial Servo、HC-SR04、SSD1306 OLED、FOC 电机、闭环直流电机、自锁继电器、ADC 模拟量 |
| 总线与抽象 | GPIO、PWM (Router)、I2C (v6 兼容)、ADC 子系统、RMT |
| 仿真 | Wasm Worker (@wink-ai/unisim)、Asyncify、PAL 物理源旁路、Protocol Bypass |
| 代码生成 | App 模板、device_tree 生成 (`wink gen`) |
| 安全 | App 静态检查、Worker watchdog、错误状态码 (`wink_status_t`) |
| 编译 | 云端/本地 Docker 与 `wink build` CLI |
| 烧录 | Chrome/Edge WebSerial/WebUSB 烧录 (`wink flash`) |
| 验证 | Golden Trace 基础事件与对比 |

### 3.2 暂缓支持

| 能力 | 暂缓原因 |
|---|---|
| RP2040 | 需要独立 toolchain 和 UF2 流程 |
| 多板通信 | 状态同步和时钟模型复杂 |
| ngspice 电路仿真 | 与行为级 MVP 价值不一致 |
| 复杂 3D 机械臂 | 美术资源和物理建模成本高 |
| Wi-Fi/BLE 云连接 | 安全和网络配置复杂 |
| 用户自定义 C 库 | 安全审计成本高 |
