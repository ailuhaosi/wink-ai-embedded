...
export class PinArbiter implements IPinArbiter {
  ...
  setDriver(pinNumber: number, driver: PinDriver): void {
    ...
    if (!pinState) {
      pinState = {
        resolvedState: LogicStates.HI_Z,
        drivers: new Map(),
      };
  ...
  readPin(pinNumber: number): LogicState {
    const pinState = this.pinStates.get(pinNumber);
    return pinState ? pinState.resolvedState : LogicStates.HI_Z;
  }

  getResolvedVoltage(pinNumber: number): number {
    const state = this.readPin(pinNumber);
    switch (state) {
      case LogicStates.HIGH: return 3.3;
      case LogicStates.LOW: return 0.0;
      case LogicStates.CONFLICT: return 1.65; // Contention mid-point (for LED brightness calculation)
      case LogicStates.HI_Z: default: return 0.0; // Floating defaults to 0V (component-specific handling can override)
    }
  }
  ...
  private resolvePinState(drivers: Map<string, PinDriver>): LogicState {
    if (drivers.size === 0) return LogicStates.HI_Z;
    ...
    for (const [, drv] of drivers) {
      if (drv.state === LogicStates.HI_Z) continue; // Hi-Z drivers don't contribute
    ...
    if (activeDrivers.length === 0) return LogicStates.HI_Z; // All drivers are Hi-Z
    ...
    if (!allAgree) return LogicStates.CONFLICT; // Contention: strongest drivers disagree
